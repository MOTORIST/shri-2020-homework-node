...
describe('settings saga', () => {
    ...
    describe('fetchSettingsSuccess', () => {
        it('run without exception', () => {
            return expectSaga(fetchSettingsSaga)
                .provide([
                    [matchers.call.fn(api.settings.getSettings), fakeSettingsRes],
                ])
        ...
        it('run with exception ', () => {
            ...
            return expectSaga(saga)
                .provide([
                    [matchers.call.fn(api.settings.getSettings), throwError(fakeError)],
                ])
                .call(api.settings.getSettings)
    ...
    describe('saveSettingsSaga', () => {
        it('run without exception', () => {
            return expectSaga(saveSettingsSaga, {values: settingsData})
                .provide([
                    [matchers.call.fn(api.settings.saveSaveSettings), fakeSettingsRes]
                ])
        ...
        it('run with exception', () => {
            const saga = expectErrorSaga(saveSettingsSaga, {values: settingsData}).toThrowError(fakeError);

            return expectSaga(saga)
                .provide([
                    [matchers.call.fn(api.settings.saveSaveSettings), throwError(fakeError)],
                ])
                .call(api.settings.saveSaveSettings, settingsData)
